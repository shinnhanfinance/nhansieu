...
### **Security Files**
Các file chứa thông tin nhạy cảm đã được di chuyển vào `configs/secrets/`:
- ✅ `configs/secrets/client_secrets.json` - Google OAuth credentials (KHÔNG commit vào Git)
- ✅ `configs/secrets/serviceAccountKey.json` - Firebase service account (KHÔNG commit vào Git)
- ⚠️ `send-email.js` - Gmail credentials (cần kiểm tra và di chuyển nếu cần)

**Lưu ý**: Thư mục `configs/secrets/` đã được thêm vào `.gitignore` và sẽ không được commit vào Git.
